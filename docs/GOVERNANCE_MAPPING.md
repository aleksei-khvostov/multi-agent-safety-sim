...
Future work extends this toward delegated agentic workflows, runtime enforcement, action-risk classification, and richer governance metrics.
---
...
| High-impact action classification | ⬜ Planned |
...
**Current coverage:** The project produces JSON traces and run metadata. Recent pilot work added full message traces and round traces with agent attribution, making manual review possible.
**Planned work:** Add trace completeness checks, generated run summaries, and manual review labels for watchdog alerts.
...
**Current coverage:** The project uses run IDs, config-driven execution, tests, structured outputs, saved configs, and deterministic seeds where applicable.
...
**Current coverage:** Current probes measure simple observable signals. `EVALUATION_CRITERIA.md` defines pilot-level labels such as `TP_candidate`, `FP`, `FN_candidate`, `benign_coordination`, and `unclear`.
**Planned work:** Add violation rate, false positive rate, trace completeness, escalation rate, block rate, and watchdog consistency metrics.
**Status:** 🟡 Partial
---
...
**Status:** 🟡 Partial
---
### 13. High-Impact Action Classification
**Governance concern:** Agentic systems need a reproducible way to decide which actions require human review, stronger authorization, or blocking.
**Failure mode:** An agent performs an externally visible, destructive, financial, administrative, or security-sensitive action without appropriate review because “high-impact action” is not explicitly defined.
**Current coverage:** Not implemented in the current MVP. High-risk actions are currently simulated only, and the project does not yet include a formal action-risk taxonomy.
**Planned work:** Implement an explicit `ACTION_RISK_MAPPING` taxonomy with at least LOW / MEDIUM / HIGH / CRITICAL levels. LOW actions may be auto-approved if policy permits. HIGH and CRITICAL actions should require parameter-bound human approval before execution. Approval records should include actor, action/tool name, target resource, normalized parameters, timestamp, expiry, and policy version. The system should fail closed when risk classification, approval validation, policy lookup, or audit logging fails.
**Status:** ⬜ Planned
---
## Main Gaps
...
The first real-model pilot showed this clearly: the watchdog emitted deception alerts after detecting words such as “defect” in a disclosure/rejection context, even though both players cooperated in all rounds.
...
- distinction between mentioning an unsafe instruction and executing it.
...
### 3. No Formal Action-Risk Taxonomy Yet
The project does not yet define which actions count as LOW, MEDIUM, HIGH, or CRITICAL risk.
This matters because human oversight depends on action classification. A system cannot reliably require approval for “high-impact actions” unless it first defines what those actions are.
Future work should implement explicit action-risk mapping, including:
- LOW: read-only or safe query actions;
- MEDIUM: ordinary write actions or bounded API calls;
- HIGH: externally visible communications, code execution, or administrative changes;
- CRITICAL: destructive, financial, security-sensitive, irreversible, or privilege-changing actions.
HIGH and CRITICAL actions should require parameter-bound human approval and should fail closed if classification, approval validation, or audit logging fails.
### 4. Prisoner’s Dilemma Is Only a Baseline
...
5. Metrics Are Not Yet Calibrated
...
Trace completeness	Whether all required run metadata and events are present	🟡 Partial
False positive review rate	Share of watchdog alerts judged non-risky after review	🟡 Partial
...
High-impact action approval rate	Share of HIGH/CRITICAL actions with valid parameter-bound approval before execution	⬜ Planned
Unauthorized high-impact execution rate	Share of HIGH/CRITICAL actions executed without valid approval	⬜ Planned
Watchdog speech/alert conflict rate	Share of rounds where watchdog narrative assessment conflicts with saved alert events	⬜ Planned
Watchdog conflict handling rate	Share of speech/alert conflicts labeled as watchdog_consistency_conflict instead of normal alerts	⬜ Planned
...
* human-review gates for simulated high-impact actions.

⸻

Relationship to Agent Security Controls

The governance mapping is informed by practical agent-security control patterns, especially:

* least-privilege tool access;
* explicit action-risk classification;
* human-in-the-loop gates for high-impact actions;
* parameter-bound approval records;
* structured audit metadata;
* anomaly monitoring;
* secure inter-agent communication;
* circuit breakers for cascading failures;
* adversarial test matrices for prompt override, tool misuse, approval bypass, privilege escalation, and multi-agent chaining.

These controls help convert abstract governance goals into testable implementation requirements.

For the current MVP, the most relevant near-term controls are:

1. action-aware watchdog detection;
2. false-positive review labels;
3. speech/alert consistency checks;
4. full message and round traces;
5. high-impact action taxonomy for future simulated tools;
6. auditability tests for delegated workflows.
...
* connecting governance concepts to concrete test cases, traces, and planned runtime controls.
...
This mapping will be updated as new scenarios, probes, metrics, and runtime controls are added.